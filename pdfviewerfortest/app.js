...
// === Load test metadata & code (tryb tymczasowy: mock, ale z prawdziwym kodem testu) ===
async function loadTest(type) {
  if (!testStatusEl || !testCodeEl) return;

  // === TRYB MOCKA ===
  // Na czas awarii backendu testy są symulowane statycznie (wyniki),
  // ale kod testu jest pobierany normalnie z /test-code/.
  // Po naprawie API możesz odkomentować oryginalny kod poniżej.
  ...
  const mockData = {
    api: { emoji: '✅', display: 'Passed 20/20', duration: 6.34 },
    selenium: { emoji: '✅', display: 'Passed 6/6', duration: 30.61 },
    cypress: { emoji: '✅', display: 'Passed 8/8', duration: 23.82 },
  };
  const mock = mockData[type] || { emoji: '⚙️', display: 'Brak danych', duration: 0 };

  // Wyświetlenie statusu (zielony box)
  testStatusEl.innerHTML = `${mock.emoji} ${mock.display} (${mock.duration.toFixed(2)}s)`;
  testStatusEl.className =
      'mb-2 p-2 rounded font-bold text-center transition-all duration-300 shadow-md bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300';

  // Pobierz prawdziwy kod testu z backendu (tylko odczyt, bez uruchamiania)
  try {
    const codeRes = await fetch(`${apiBase}/test-code/${type}`);
    if (!codeRes.ok) {
      testCodeEl.textContent = `⚠️ Nie znaleziono pliku testu (${type}). Kod ${codeRes.status}`;
      return;
    }
    ...
    testCodeEl.textContent = `⚠️ Błąd pobierania kodu testu:\n${String(err)}`;
  }

  /*
  // === Oryginalny kod (odkomentuj po naprawie backendu) ===
  testStatusEl.textContent = 'Loading...';
  testStatusEl.className = 'mb-2 p-2 rounded font-bold text-center text-gray-500 bg-gray-200 dark:bg-gray-700 shadow-inner';
...
// === Rerun Test (tymczasowo zablokowany w trybie mocka) ===
rerunBtn.addEventListener('click', (e) => {
  ...
  testStatusEl.textContent = 'Tryb demonstracyjny — testy nie są uruchamiane.';
  testStatusEl.className = 'mb-2 p-2 rounded font-bold text-center bg-yellow-100 dark:bg-yellow-800 text-yellow-700 dark:text-yellow-300 shadow-md transition-all duration-300';
});
...
rerunBtn.addEventListener('click', async () => {
  // === Tymczasowo wyłączony przycisk Rerun ===
  rerunBtn.disabled = true;
  rerunBtn.classList.add('opacity-50', 'cursor-not-allowed');
  rerunBtn.textContent = 'Retest niedostępny 🔒';

// --- Po naprawie backendu do usuniecia powyższy blok i przywróć event listener: ---
// rerunBtn.addEventListener('click', async () => { ... }
